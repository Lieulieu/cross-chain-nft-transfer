...
contract XNFT is ERC721, ERC721URIStorage, ERC721Burnable, IAny2EVMMessageReceiver, ReentrancyGuard, OwnerIsCreator {
    ...
    // Saigon Testnet = 2021
    uint256 constant FIX_CHAIN_ID = 2021;
    ...
    modifier onlyOnFixChain() {
        if (block.chainid != FIX_CHAIN_ID) {
            revert OnlyOnFixChain();
        }
        _;
    }
    ...
    constructor(address ccipRouterAddress, address linkTokenAddress, uint64 currentChainSelector)
        ERC721("Chainlink Cross-Chain NFT", "CCNFT")
    {
    ...
    function mint() external onlyOnFixChain {
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{',
                            '"name": "Cross-Chain NFT #', currentId.toString(), '",',
                            '"description": "Chainlink Cross-Chain NFT Transfer.",',
